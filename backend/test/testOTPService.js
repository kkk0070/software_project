...
console.log(`  ✅ OTPs are 6 digits: ${otp1.length === 6 && otp2.length === 6}`);
...
console.log(`  ✅ Valid OTP verification: ${validResult.success}`);
...
console.log(`  ✅ Invalid OTP rejected: ${!invalidResult.success}`);
...
for (let i = 1; i <= 6; i++) {
  ...
  if (i === 5) {
    console.log(`  ✅ Blocked after 5 attempts: ${!result.success && result.message.includes('Too many')}\n`);
  }
}
...
console.log(`  ✅ Secrets are base64: ${/^[A-Za-z0-9+/=]+$/.test(secret1)}\n`);

console.log('✅ All OTP Service tests completed successfully!');
