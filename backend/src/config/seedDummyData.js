  ...
  try {
    ...
    if (parseInt(rideCount.rows[0].count) === 0) {
      const locations = [
        ...
        { name: 'Peelamedu', lat: 11.0267, lng: 77.0102 },
        { name: 'RS Puram', lat: 11.0116, lng: 76.9515 },
        { name: 'Ramanathapuram', lat: 11.0022, lng: 76.9942 },
        { name: 'Saravanampatti', lat: 11.0772, lng: 76.9922 }
      ];
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const distance = (Math.random() * 15 + 1).toFixed(2); // Slightly shorter distances for city
        const fare = (distance * 12 + 40).toFixed(2); // Rupee-based pricing
        const carbonSaved = (distance * 0.21).toFixed(3);
        const duration = Math.floor(distance * 4 + 10);
    ...
    if (parseInt(incidentCount.rows[0].count) === 0) {
      ...
      for (let i = 0; i < 25; i++) {
        ...
        `, [
          ...
          'Gandhipuram Area',
          11.0168 + (Math.random() - 0.5) * 0.05,
          76.9558 + (Math.random() - 0.5) * 0.05,
