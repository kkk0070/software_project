...
const GPSLogs = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Map of Anomalies */}
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Anomaly Locations</h3>
        <div className="bg-gray-100 rounded-lg h-96 overflow-hidden">
          <MapComponent
            markers={anomalies.map(a => ({
              position: { lat: 11.0168 + (Math.random() - 0.5) * 0.05, lng: 76.9558 + (Math.random() - 0.5) * 0.05 }, // Simulated coords
              title: `${a.issue} - ${a.vehicle}`
            }))}
            paths={anomalies.map((a, i) => {
              const startLat = 11.0168 + (Math.random() - 0.5) * 0.05;
              const startLng = 76.9558 + (Math.random() - 0.5) * 0.05;
              ...
              return {
                coords: [
                  ...
                  { lat: (startLat + endLat) / 2 + 0.002, lng: (startLng + endLng) / 2 },
            ...
            {anomalies.map((anomaly) => (
              <div key={anomaly.id} className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${anomaly.severity === 'High' ? 'bg-red-100 text-red-800' :
              ...
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="mt-3">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${region.avgAccuracy >= 85 ? 'bg-green-500' :
              ...
              {[
                ...
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${log.status === 'Resolved' ? 'bg-green-100 text-green-800' :
