          ...
          <div className="bg-gray-100 rounded-lg h-96 overflow-hidden">
            <MapComponent
              markers={activeRides.map(ride => ({
                position: { lat: ride.current_lat || 11.0168, lng: ride.current_lng || 76.9558 },
                title: `Ride #${ride.id} - ${ride.status}`
              }))}
              paths={activeRides.map(ride => {
                const currentLat = ride.current_lat || 11.0168;
                const currentLng = ride.current_lng || 76.9558;
                return {
                  coords: [
                    { lat: currentLat - 0.01, lng: currentLng - 0.01 }, // Simulated Pickup
                    { lat: currentLat - 0.003, lng: currentLng - 0.001 }, // Mid Point
                    { lat: currentLat, lng: currentLng } // Current Position
                  ],
                  color: '#10b981',
                  weight: 4
                };
            ...
            {incidents.map((incident) => (
              <div key={incident.id} className="flex items-start p-4 border border-gray-200 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full inline-block mt-2 ${incident.priority === 'Critical' || incident.priority === 'High' ? 'bg-red-100 text-red-800' :
