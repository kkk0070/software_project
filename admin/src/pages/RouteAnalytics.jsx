    ...
    <div className="space-y-6">
            ...
            <tbody>
              {routeComparison.map((route, index) => (
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${route.efficiency >= 94 ? 'bg-green-100 text-green-800' :
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        ...
        <div className="bg-gray-100 rounded-lg h-96 overflow-hidden">
          <MapComponent
            markers={popularRoutes.map((r, i) => ({
              position: { lat: parseFloat(r.pickup_lat), lng: parseFloat(r.pickup_lng) },
              title: r.pickup_location
            }))}
            paths={popularRoutes.slice(0, 5).map((r, i) => ({
              coords: [
                { lat: parseFloat(r.pickup_lat), lng: parseFloat(r.pickup_lng) },
                { lat: parseFloat(r.dropoff_lat), lng: parseFloat(r.dropoff_lng) }
              ],
              color: i === 0 ? '#10b981' : i === 1 ? '#3b82f6' : '#8b5cf6',
              weight: 6,
              real: true // This will trigger real road direction simulation in MapComponent
            }))}
          />
        </div>
      </div>
